...
export default function ClientesPage(){
  ...
  const q = useQuery({ queryKey:['clientes', filtro, mensalista], queryFn:() => apiGet(`/api/clientes?pagina=1&tamanho=20&filtro=${encodeURIComponent(filtro)}&mensalista=${mensalista}`) })
    ...
    <div>
      ...
      <div style={{marginTop:12}}>
        ...
        <div style={{display:'grid', gap:6, gridTemplateColumns:'repeat(3, 1fr)'}}>
          ...
          <label><input type="checkbox" checked={form.mensalista} onChange={e=>setForm({...form, mensalista:e.target.checked})}/> Mensalista</label>
          ...
          <tbody>
            {q.data.itens.map(c=>(
              <tr key={c.id}>
                ...
                <td><button onClick={()=>remover.mutate(c.id)}>Excluir</button></td>
              </tr>
            ))}
          </tbody>
        </table>
